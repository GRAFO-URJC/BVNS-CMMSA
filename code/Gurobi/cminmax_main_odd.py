...
from gurobipy.gurobipy import Model, GRB, min_, abs_, quicksum
...
class Odd:
    ...
    def run(self):
        ...
        NUM_VERTICES = len(V)
        # GENERATE A LIST OF DISTANCEES
        D = [d for d in range(1, math.floor(NUM_VERTICES / 2) + 1)]

        # GENERATE AJDAENCY MATRIX WITH WEIGHTS 0,+1,-1
        ...
        m.addConstrs((is_on_the_right[u, v] == abs_(aux_is_on_the_right[u, v])) for u, v in product(V, V) if u != v)
        ...
        m.addConstrs(
            (distance_left[u, v] == cycle_distance[u, v] * (1 - is_on_the_right[u, v])) for u, v in product(V, V) if u != v)

        print("Constraint 13: Error left")
        m.addConstrs(((distance_left[u, v] * (1 - is_on_the_right[u, w]) - distance_left[u, w] * (1 - is_on_the_right[u, v])
        ...
        m.addConstrs(((distance_right[u, v] * is_on_the_right[u, w] - distance_right[u, w] * is_on_the_right[u, v]
                    + error[u, v, w] * NUM_VERTICES >= 0)
